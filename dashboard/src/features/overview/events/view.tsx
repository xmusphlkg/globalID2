        ...
        <section className="app-panel overflow-hidden">
          <div className="flex flex-col gap-3 border-b border-[#E5E5E2] p-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-wrap items-end gap-2">
              ...
              <label className="grid gap-1 text-xs text-[#6B7280]"><span>Filter</span><select value={stateFilter} onChange={event => setStateFilter(event.target.value)} className="h-9 rounded-md border border-[#D9D9D6] bg-white px-2 text-sm"><option value="">All</option>{filterOptions(activeTab).map(option => <option key={option} value={option}>{pretty(option)}</option>)}</select></label>
...
function ReportsTable({ rows, compareLeft, compareRight, comparison, setCompareLeft, setCompareRight, onCompare, onRollback, busy }: { rows: ReportRow[]; compareLeft: string; compareRight: string; comparison: CompareResponse | null; setCompareLeft: (value: string) => void; setCompareRight: (value: string) => void; onCompare: () => void; onRollback: (row: ReportRow) => void; busy: boolean }) {
  return <>
    <div className="grid gap-2 border-b border-[#E5E5E2] p-4 lg:grid-cols-[1fr_1fr_auto]"><select value={compareLeft} onChange={event => setCompareLeft(event.target.value)} className="h-9 min-w-0 rounded border border-[#D9D9D6] bg-white px-2 font-mono text-xs">{rows.map(row => <option key={`left:${row.report_id}`} value={row.report_id}>{row.report_id}</option>)}</select><select value={compareRight} onChange={event => setCompareRight(event.target.value)} className="h-9 min-w-0 rounded border border-[#D9D9D6] bg-white px-2 font-mono text-xs">{rows.map(row => <option key={`right:${row.report_id}`} value={row.report_id}>{row.report_id}</option>)}</select><button disabled={busy || !compareLeft || !compareRight || compareLeft === compareRight} onClick={onCompare} className="inline-flex h-9 items-center justify-center gap-2 rounded border border-[#D9D9D6] px-3 text-sm font-semibold disabled:opacity-40"><GitCompareArrows className="h-4 w-4" />Compare</button></div>
