...
export default function ResearchAskSurveillance({ initialLanguage = 'en' }: { initialLanguage?: 'en' | 'zh' }) {
  ...
  const geographyLabel = countryCodes.length === 1
    ...
    : countryCodes.length > 1
      ? (lang === 'zh' ? `${countryCodes.length} 个相关国家/地区` : `${countryCodes.length} related locations`)
      : (lang === 'zh' ? '报告国家/地区' : 'reporting locations');
    ...
    <div className="ask-surveillance-widget" data-research-surveillance="true">
      <div className="ask-surveillance-widget-header">
        <div className="ask-surveillance-scope">
          ...
          <span>{lang === 'zh' ? '已匹配监测序列' : 'Matched surveillance series'}</span>
        </div>
        ...
        <p>
          {lang === 'zh'
            ...
            : 'This GIDS reported-case time series adds current surveillance context to the literature evidence above.'}
        </p>
        <div className="ask-surveillance-links">
          <a href={diseaseHref}>
            {lang === 'zh' ? '打开疾病数据页' : 'Open disease data'} <span aria-hidden="true">↗</span>
          </a>
          <a href={countryHref}>
            {lang === 'zh' ? '查看地区资料' : 'View location profile'} <span aria-hidden="true">↗</span>
          </a>
        </div>
      </div>
      <DiseaseCountryCurve
      ...
      <p className="ask-surveillance-note">
        {lang === 'zh'
          ...
          : 'Interpretation note: this is a reported-case curve, not a causal conclusion from the literature or a risk forecast. Source, cadence, and provisional-data notes are available below the chart.'}
      </p>
    </div>
  );
}
