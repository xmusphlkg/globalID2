...
export default function ResearchHotspotCharts({ hotspots }: Props) {
  ...
  const heatmapOption = useMemo<EChartsOption>(() => {
    ...
    return {
      ...
      grid: { top: 24, right: 36, bottom: 84, left: 190 },
      xAxis: {
        type: 'category',
        data: xLabels,
      ...
      dataZoom: [
        ...
        { type: 'slider', xAxisIndex: 0, height: 18, bottom: 4, borderColor: colors.line, textStyle: { color: colors.faint } },
      ],
      toolbox: {
        right: 8,
        top: 0,
  ...
  const burstOption = useMemo<EChartsOption>(() => {
    ...
    return {
      ...
      grid: { top: 22, right: 44, bottom: 48, left: 178 },
      xAxis: {
        type: 'value',
      ...
      yAxis: {
        ...
        inverse: true,
        data: yLabels,
        axisLabel: { color: colors.muted, width: 158, overflow: 'truncate' },
        axisTick: { show: false },
        axisLine: { lineStyle: { color: colors.line } },
      },
      dataZoom: bursts.length > 8 ? [
        { type: 'inside', yAxisIndex: 0, filterMode: 'none' },
        { type: 'slider', yAxisIndex: 0, width: 16, right: 4, borderColor: colors.line, textStyle: { color: colors.faint } },
      ] : [],
      toolbox: {
        right: 8,
        top: 0,
  ...
  const migrationOption = useMemo<EChartsOption>(() => {
    const paletteByTopic = new Map((hotspots.alluvial?.topics ?? []).map((topic, index) => [topic.slug, colors.palette[index % colors.palette.length]]));
    ...
    const data = alluvialNodes.map((node) => ({
      ...
      topic: node.topic,
      count: node.count,
      itemStyle: { color: paletteByTopic.get(node.slug) ?? colors.palette[0] },
      label: {
        color: colors.text,
        formatter: `${periodShort(node.period)}\n${node.topic}`,
      },
    }));
    const links = alluvialLinks
      .map((link) => ({
        ...
        lineStyle: { color: paletteByTopic.get(link.slug) ?? colors.palette[0], opacity: 0.32 },
      }))
      ...
      tooltip: {
        ...
        formatter: (param: any) => {
          ...
          if (param?.dataType === 'edge') {
            return `<strong>${escapeHtml(data.topic)}</strong><br/>${escapeHtml(data.source)} → ${escapeHtml(data.target)}<br/>${data.source_count ?? 0} → ${data.target_count ?? 0} ${t('mentions', '次提及')}`;
          }
          return `<strong>${escapeHtml(data.topic)}</strong><br/>${escapeHtml(data.period)} · ${data.count ?? 0} ${t('mentions', '次提及')}`;
        },
      },
      toolbox: {
        right: 8,
  ...
  }, [alluvialLinks, alluvialNodes, colors, hotspots.alluvial?.topics, lang]);
  ...
  return (
    <div className="research-hotspots-widget">
      ...
      <ChartFrame
        ...
        toolbar={toolbar}
        chart={({ isFullscreen }) => (
          <div className="research-hotspot-plot" style={{ height: isFullscreen ? '100%' : activeHeight }}>
