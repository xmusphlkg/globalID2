"""Korea KDCA monthly crawl orchestration."""
...
def history_start_year(task, updater) -> int:
    """Resolve and clamp the configured KDCA history boundary."""
    raw_value = (
    ...
    *,
    ...
    await task_manager.add_workbook_entry(
        ...
        content=(
            ...
            f"History Start Year: {history_start_year(task, updater)}\n"
    ...
    if force:
        mode_text = f"full history refresh from {start_year}"
    elif fill_missing:
    ...
    if force:
        months_to_fetch = updater.history_months(start_year=start_year)
    elif fill_missing:
        now_dt = datetime.now()
        recent: list = []
        ...
        for year in range(start_year, now_dt.year + 1):
            for month in range(1, 13 if year < now_dt.year else now_dt.month + 1):
    ...
    if not process:
        ...
        summary_message = (
            f"Korea KDCA data upserted: {imported} rows across "
        ...
        summary_message = (
            "Korea KDCA source refreshed; no rows matched disease mapping "
