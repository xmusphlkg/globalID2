#!/usr/bin/env python3
"""Fetch and import Switzerland FOPH IDD historical case data.

This script is additive: it upserts Switzerland country metadata, standard
disease rows, CH mappings, and CH disease_records without clearing data for
other countries.
"""
...
CH_MAPPING_CSV = ROOT / "configs" / "mapping" / "ch.csv"
...
async def _ensure_ch_country_and_scope(db) -> None:
    profile = get_country_profile("CH")
    bootstrap = get_country_bootstrap_config("CH")
    ...
    await db.execute(
        ...
        ),
        {
            ...
            "disease_mapping_rules": json.dumps(bootstrap.get("disease_mapping_rules") or {}),
            "report_config": json.dumps(bootstrap.get("report_config") or {}),
            "metadata": json.dumps({"origin": "ch_history_import"}),
    ...
    await ensure_country_scope(
        ...
        is_default=True,
        is_active=True,
        metadata={"origin": "ch_history_import"},
    )
...
async def _upsert_ch_mappings(db) -> int:
    await db.execute(text("ALTER TABLE disease_mappings ALTER COLUMN category DROP NOT NULL"))

    with CH_MAPPING_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    ...
    for row in rows:
        ...
        source = _norm(row.get("data_source")) or "Switzerland FOPH IDD"
        metadata = {
            "origin": "ch_mapping_csv",
...
async def _db_ch_summary(db) -> dict:
...
async def run(args: argparse.Namespace) -> None:
    ...
    logger.info("Preparing CH country, disease, and mapping rows...")
    async with get_db() as db:
        await _ensure_ch_country_and_scope(db)
        standard_count = await _upsert_standard_diseases_and_diseases(db)
        mapping_count = await _upsert_ch_mappings(db)
        await db.commit()
    logger.info(
        "CH prerequisites ready | standard_diseases={} mapping_entries={}",
        standard_count,
        mapping_count,
    )

    logger.info(
        "Fetching CH IDD history | start_year={} current_year={}",
        args.start_year,
        datetime.now().year,
    )
    updater = CHMonthlyUpdater(output_csv=output_csv)
    ...
    logger.info(
        "CH source prepared | rows={} latest={} csv={} version={}",
        len(fetched.rows),
    ...
    async with get_db() as db:
        before = await _db_ch_summary(db)
    ...
    async with get_db() as db:
        after = await _db_ch_summary(db)

    print("\nCH history import complete")
    print(f"Source CSV: {fetched.source_csv}")
    print(f"IDD version: {fetched.version or 'unknown'}")
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--raw-dir", default=str(ROOT / "data" / "raw" / "ch"))
