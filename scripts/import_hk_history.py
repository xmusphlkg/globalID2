#!/usr/bin/env python3
"""Fetch and import Hong Kong, China CHP monthly historical data.

This script is additive: it upserts Hong Kong, China country metadata, standard
disease rows, HK mappings, and HK disease_records without clearing data for
other countries.
"""
...
from src.data.processors.hk import DEFAULT_OUTPUT_CSV, HKMonthlyUpdater

logger = get_logger(__name__)

STANDARD_DISEASES_CSV = ROOT / "configs" / "standard_diseases.csv"
HK_MAPPING_CSV = ROOT / "configs" / "mapping" / "hk.csv"
...
async def _ensure_hk_country_and_scope(db) -> None:
    profile = get_country_profile("HK")
    bootstrap = get_country_bootstrap_config("HK")
    ...
    await db.execute(
        ...
        ),
        {
            ...
            "metadata": json.dumps({"origin": "hk_history_import"}),
    ...
    await ensure_country_scope(
        ...
        is_default=True,
        is_active=True,
        metadata={"origin": "hk_history_import"},
    )
...
async def _upsert_hk_mappings(db) -> int:
    await db.execute(text("ALTER TABLE disease_mappings ALTER COLUMN category DROP NOT NULL"))

    with HK_MAPPING_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    ...
    for row in rows:
        ...
        source = _norm(row.get("data_source")) or "Hong Kong, China CHP"
        metadata = {
            "origin": "hk_mapping_csv",
    ...
    await db.execute(
        text(
            ...
            INSERT INTO disease_mappings (
                ...
                :disease_id, 'HK', :local_name, 'SRC_HK_CHP', :is_primary, :is_alias,
...
async def _db_hk_summary(db) -> dict:
...
async def run(args: argparse.Namespace) -> None:
    ...
    logger.info("Preparing HK country, disease, and mapping rows...")
    async with get_db() as db:
        await _ensure_hk_country_and_scope(db)
        standard_count = await _upsert_standard_diseases_and_diseases(db)
        mapping_count = await _upsert_hk_mappings(db)
        await db.commit()
    logger.info(
        "HK prerequisites ready | standard_diseases={} mapping_entries={}",
        standard_count,
        mapping_count,
    )

    logger.info(
        "Fetching HK CHP monthly history | months={} range={}-01..{}-{:02d}",
        len(months),
        args.start_year,
        now.year,
        now.month,
    ...
    logger.info(
        "HK source prepared | rows={} latest={} csv={}",
        len(fetched.rows),
    ...
    async with get_db() as db:
        before = await _db_hk_summary(db)
    ...
    async with get_db() as db:
        after = await _db_hk_summary(db)

    print("\nHK history import complete")
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--raw-dir", default=str(ROOT / "data" / "raw" / "hk"))
